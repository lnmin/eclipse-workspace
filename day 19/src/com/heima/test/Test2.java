...
public class Test2 {
	 ...
	 * 分析：
	 * 1，创建键盘录入对象
	 * 2，将键盘录入的结果，存储在String类型字符串中，存储int类型中如果有不符合条件的直接报错，无法进行后续判断
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			try {
			    ...
			    System.out.println(Integer.toBinaryString(num));    //将整数转为二进制
			    break;                                              //跳出循环
			}catch(Exception e) {
				try {
					new BigInteger(line);
					System.out.println("录入错误，您录入的是一个过大的整数，请重新输入一个整数：");
				} catch (Exception e2) {                         //alt + shift + z (try catch快捷键)
